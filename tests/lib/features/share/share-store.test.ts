...
describe('ShareStore', () => {
    ...
    it('should generate link', async () => {
        ...
        vi.spyOn(DomElementLocator, 'createDescriptor').mockReturnValue({ type: 'id', val: 'test-id' } as any);
        vi.spyOn(DomElementLocator, 'serialize').mockReturnValue('serialized-id');

        await store.generateLink();

        expect(navigator.clipboard.writeText).toHaveBeenCalledWith(expect.stringContaining('cv-show=serialized-id'));
    });
});
