...
// Mock dependencies
vi.mock('../../src/core/stores/placeholder-value-store.svelte', () => ({
...
import { placeholderValueStore } from '../../src/core/stores/placeholder-value-store.svelte';
...
describe('DataStore', () => {
    ...
    beforeEach(async () => {
        ...
        const module = await import('../../src/core/stores/main-store.svelte');
    ...
    describe('Initialization', () => {
         it('should register placeholders from config', () => {
             const config = {
                 placeholders: [
                     { name: 'p1', defaultValue: 'default' }
                 ]
             };
             
             initStore(config);
             
             expect(placeholderRegistryStore.register).toHaveBeenCalledWith(expect.objectContaining({
                 name: 'p1',
                 source: 'config'
             }));
         });
